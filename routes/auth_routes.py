...
#                 usuario = repo_usuario.autenticar_usuario(email, password)

#             if usuario:
#                 nombre_rol = usuario.get("rol", "").lower()
#                 if not nombre_rol:
#                     flash("Error al obtener el rol del usuario.", "danger")
#                     logger.error(f"Error al obtener 'rol' para el usuario: {email}")
#                     return redirect(url_for("auth_routes.login"))

#                 session["user_role"] = nombre_rol
#                 session["user_email"] = email
#                 logger.info(f"Rol del usuario en sesión: {nombre_rol}")
#                 return redirect(url_for(role_redirect(nombre_rol)))
#             else:
#                 flash("Credenciales incorrectas.", "danger")
#                 logger.warning(f"Inicio fallido: {email}")
#         except Exception as e:
